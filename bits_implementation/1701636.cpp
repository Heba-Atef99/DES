...
const int DEFAULT_SIZE = 64, IP_OUT = 64, RIP_OUT = 64, PC1_OUT = 56, PC2_OUT = 48, EP_OUT = 48, P_OUT = 32;

// Functions Identifiers
u64 permutation(u64 input, int in_size, int out_size, char p_type);
u64 circularLeftShift(u64 k, int n);
u64 SBOX(u64 t);
u64 swap(u64 l, u64 r);

int main(int argc, char** argv)
{
    //Variables declarations
    string operation, input, key, output, lable;
    u64 _input = 0, _output = 0, _key = 0;
    ...
    if (operation == "encrypt" || operation == "decrypt")
    {
        ...
        _input = stoull(input, nullptr, 16);
        _key = stoull(key, nullptr, 16);

        //perform permutations on the key
        _key = permutation(_key, DEFAULT_SIZE, PC1_OUT, '1'); //Permuted Choice 1

        //split the key
        u64 C = (_key & 0xFFFFFFF0000000) >> 28;
        u64 D = _key & 0xFFFFFFF;
        ...
        for (int i = 0; i < 16; i++)
        {
            ...
            //concatenate the splited keys
            _key = (C << 28) | D;
            _key = permutation(_key, PC1_OUT, PC2_OUT, '2'); //Permuted Choice 2
        ...
        _input = permutation(_input, DEFAULT_SIZE, IP_OUT, 'i'); //Initial Permutation
        
        //split the input
        u64 L = (_input & 0xFFFFFFFF00000000) >> 32;
        u64 R = _input & 0xFFFFFFFF;
        u64 F = 0;

        //start the rounds
        for (int i = 0; i < 16; i++)
        {
            _key = (operation == "encrypt") ? keys[i] : keys[16 - i - 1];

            F = permutation(R, IP_OUT / 2, EP_OUT, 'e'); //Expansion Permutation
            F = F ^ _key;//XOR
            F = SBOX(F);//S-Box
            F = permutation(F, P_OUT, P_OUT, 'p'); //Permutation
            F = F ^ L;//XOR
        ...
        _output = swap(L, R);
        _output = permutation(_output, RIP_OUT, RIP_OUT, 'r'); //Inverse Initial Permutation
...
u64 SBOX(u64 t)
{
    u64 output = 0, s_input = 0;
    int col = 0, row = 0, s = 0;
    for (int i = 6, s_num = 1; s_num <= 8; i += 6, s_num++)
    {
        s_input = (t >> (EP_OUT - i)) & 0x3F;

        col = (s_input & 0x1E) >> 1;
        row = (s_input & 0x1) | (s_input >> 4 & 0x2);
...
u64 circularLeftShift(u64 k, int n)
{
    char shifted_bits = k >> (PC1_OUT / 2 - n);
    k = (k << n) | shifted_bits;
    k &= 0xFFFFFFF;
    return k;
}